  ...
  <div class="page__bd">
    ...
    <div v-show="get_token!=''">
     ...
     <div class="video_js">
      ...
      <div class="video_js_bottom" v-if="lists.video_url!=null" @click="showVideourl = !showVideourl">
        视频
      </div>
     </div>
      ...
      <div class="page__bd_spacing" style="margin-top:30rpx;">
        <div class="weui-cell">
          ...
          <div class="weui-cell__bd fw500">品牌亮点</div>
        </div>
      </div>
      <div class="weui-form-preview btn2em ">
        <div class="weui-form-preview__bd">
          ...
          <div class="weui-form-preview__item">
            <div class="weui-form-preview__label" style="color:#1a1a1a">店型：</div>
            <div class="weui-form-preview__value" style="color:#92969c">{{lists.target_stores}}</div>
          </div>
          <div class="weui-form-preview__item">
            <div class="weui-form-preview__label" style="color:#1a1a1a">投资额度：</div>
            <div class="weui-form-preview__value" style="color:#92969c">{{lists.invest_money}}</div>
          </div>
                <div class="weui-form-preview__item">
            <div class="weui-form-preview__label" style="color:#1a1a1a">收益：</div>
            <div class="weui-form-preview__value" style="color:#92969c">{{lists.profit}}</div>
          </div>
                <div class="weui-form-preview__item">
            <div class="weui-form-preview__label" style="color:#1a1a1a">回报：</div>
      ...
      <!-- 项目介绍 -->
      <div class="weui-cells weui-cells_after-title btn2em" style="margin-top:30rpx;" v-if="lists.commcontent">
        <div class="weui-cell">
          <div class="weui-cell__hd">
            <div class="detail_one"></div>
          </div>
        ...
        <div class="weui-panel weui-panel_access">
          ...
          <div class="weui-panel__bd" v-for='(comm,inded) in comment' :key="inded">
            ...
            <div style="padding:0px 30rpx">
              ...
              <!-- <div class="huozan" v-for="(comlist,indexcm) in comm.comlist" :key="indexcm" style="font-size:14px">
                <p style="display:inline-block" class="c1a1a"> {{comlist.user_nickname}}回复{{comlist.puser_nickname}}: </p>{{comlist.content}}
              </div> -->
              <div class="huozan" v-for="(comlist,indexcm) in comm.comlist" :key="indexcm" style="font-size:14px;overflow: hidden;" @click="discuss(comm.obj_id,comlist.user_id,comm.id,'send')">
                <div style="display:inline-block;color:#223f75"> {{comlist.user_nickname}}<p style="color:#92969c;display:inline-block;word-break:break-all;">回复</p>{{comlist.puser_nickname}}: </div>
    ...
    <navigator :url="'/pages/information/websorket/main?r_id='+d_user.id+'&userAvatar='+d_user.user_avatar+'&user_nickname='+d_user.user_nickname" open-type="navigate" class="weui-cell__bd" style="position:fixed;top:50vh;right:0rpx;width:20px;height:80px;display:block;background:#d61518;color:#fff;text-align:center;padding:7rpx 12rpx;border-top-left-radius: 8rpx;border-bottom-left-radius: 8rpx">
      <div style="font-size:12px;">立即沟通</div>
    </navigator>
    <div v-if="showVideourl==true">
       <video-js @hidevideo="hidevideo" :videoUrl="lists.video_url"></video-js>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    //   this.$http.post('favorite/objsharesadd', { lei: 2, obj_id: this.objId, usercode: code }).then(res => {
    ...
    getData(id) {
      this.$http.post("obj/details", { id: id }).then(res => {
        ...
        wx.setNavigationBarTitle({
          title:res.data.data.list.obj_name
        })
      });
    },
    // 获取投标
